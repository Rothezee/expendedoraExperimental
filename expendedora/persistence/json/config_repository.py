...
DEFAULT_PROMOCIONES = {
    "Promo 1": {"precio": 0.0, "fichas": 0},
    "Promo 2": {"precio": 0.0, "fichas": 0},
    "Promo 3": {"precio": 0.0, "fichas": 0},
}
...
DEFAULT_UPDATER_CONFIG = {
    ...
    "preserve_files": list(_UPDATER_PRESERVE_FILES),
}
...
DEFAULT_HARDWARE_CONFIG = {
    ...
    "esp32": {
        ...
        "debug_motor_sensor": False,
    },
}


def _normalize_counter_domains(payload: dict | None) -> tuple[dict, dict]:
    ...
    for candidate in global_candidates:
        ...
        for key, value in normalized.items():
            try:
                ...
                continue
    partial_src = raw.get(COUNTER_DOMAIN_PARTIAL) or raw.get(LEGACY_COUNTER_DOMAIN_PARTIAL) or {}
    return global_base, Counters.from_dict(partial_src).to_dict()
...
class ConfigRepository:
    ...
    @staticmethod
    def _mysql_password_str(raw_cfg: Dict[str, Any], default: str) -> str:
        """Null explícito en JSON = sin contraseña; clave ausente usa el default del perfil."""
        if "password" in raw_cfg and raw_cfg.get("password") is None:
            return ""
        return ConfigRepository._mysql_scalar_str(raw_cfg.get("password"), default)
    ...
    def mysql_connection_params(profile: Dict[str, Any] | None) -> Dict[str, Any]:
        ...
        return {
            ...
            "password": ConfigRepository._mysql_password_str(profile, d["password"]),
    ...
    def normalize(self, config: Dict[str, Any] | None) -> Dict[str, Any]:
            ...
            normalized_hoppers.append(
                {
                    ...
                    "sensor_blocked_high": bool(
                        hopper.get("sensor_blocked_high", fallback.get("sensor_blocked_high", False))
                    ),
        ...
        promociones_raw = config.get("promociones", {})
        if not isinstance(promociones_raw, dict):
            promociones_raw = {}
        normalized_promociones: Dict[str, Any] = {}
        for promo_name, default_promo in DEFAULT_PROMOCIONES.items():
            raw_promo = promociones_raw.get(promo_name, default_promo)
            if not isinstance(raw_promo, dict):
                raw_promo = {}
            normalized_promociones[promo_name] = {
                "precio": self._safe_float(raw_promo.get("precio", default_promo["precio"]), default_promo["precio"], minimum=0),
                "fichas": self._safe_int(raw_promo.get("fichas", default_promo["fichas"]), default_promo["fichas"], minimum=0),
            }
        ...
        if any(key in mysql for key in ("host", "port", "user", "password", "database")):
            legacy_conn = {
                ...
                "password": self._mysql_password_str(mysql, DEFAULT_MYSQL_CONFIG["password"]),
            ...
            mysql_cfg = {
                ...
                "local": {
                    ...
                    "password": self._mysql_password_str(local_cfg_raw, DEFAULT_MYSQL_MULTI_CONFIG["local"]["password"]),
                ...
                "production": {
                    ...
                    "password": self._mysql_password_str(prod_cfg_raw, DEFAULT_MYSQL_MULTI_CONFIG["production"]["password"]),
        ...
        merged["promociones"] = normalized_promociones
        merged["valor_ficha"] = self._safe_float(config.get("valor_ficha", 1.0), 1.0, minimum=0.01)
        ...
        if not isinstance(hardware, dict):
            hardware = {}
        esp32 = hardware.get("esp32", hardware.get("serial", {}))
        ...
        cnt_global, cnt_partial = _normalize_counter_domains(config)
    ...
    def load(self) -> Dict[str, Any]:
        ...
        if os.path.exists(self.config_path):
            ...
            if isinstance(loaded, dict):
                data = loaded
        local_path = CONFIG_LOCAL_FILE
        if os.path.exists(local_path):
            with open(local_path, "r", encoding="utf-8") as file_obj:
                local_data = json.load(file_obj)
            if isinstance(local_data, dict):
                data = _deep_merge(data, local_data)
        self._apply_mysql_env_overrides(data)
        return self.normalize(data if data else {"promociones": {}, "valor_ficha": 1.0})

    @staticmethod
    def _apply_mysql_env_overrides(data: dict) -> None:
        import os as _os

        mysql = data.setdefault("mysql", {})
        if not isinstance(mysql, dict):
            return
        for profile in ("local", "production"):
            section = mysql.get(profile, {})
            if not isinstance(section, dict):
                continue
            prefix = f"MYSQL_{profile.upper()}_"
            for field in ("host", "user", "password", "database"):
                env_val = _os.environ.get(f"{prefix}{field.upper()}")
                if env_val:
                    section[field] = env_val
