...
def format_db_exception(exc: BaseException, _depth: int = 0) -> str:
    ...
    if trivial or not rep or rep == "0":
        return (
            ...
            'Revisá expendedora/persistence/json/config.json → sección "mysql", objetos "local" y "production".'
        )
    detail = f"{type(exc).__name__}: {rep}"
    if detail and detail != "0":
        return detail
    return (
        'Fallo al conectar MySQL. Revisá expendedora/persistence/json/config.json → "mysql"."local" y "mysql"."production".'
    )
