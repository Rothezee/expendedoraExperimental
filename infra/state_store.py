...
LEGACY_BUFFER_FILE = "buffer_state.json"
SCHEMA_VERSION = 1
...
def build_snapshot(
    *,
    ...
    return {
        ...
        "reason": reason,
        "buffer": buf,
        COUNTER_DOMAIN_GLOBAL: CounterService.ensure_schema(
            contadores_global or contadores or contadores_apertura
        ),
        COUNTER_DOMAIN_PARTIAL: CounterService.ensure_schema(
...
def save_snapshot(
    ...
    *,
    ...
    if sync_config is not None:
        ...
        cnt_global = CounterService.ensure_schema(
            snap.get(COUNTER_DOMAIN_GLOBAL) or snap.get(LEGACY_COUNTER_DOMAIN_GLOBAL)
        )
        cnt_partial = CounterService.ensure_schema(
...
def recover_state(
    ...
    registro_path: str = "registro.json",
    ...
    if legacy_buffer:
        ...
        if "fichas_restantes" in legacy_buffer:
